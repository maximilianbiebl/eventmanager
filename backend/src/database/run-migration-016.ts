...
async function runMigration() {
  ...
  try {
    console.log('Starting migration 016: Unique user name...');


    const migrationSQL = fs.readFileSync(
      path.join(__dirname, 'migrations', '016_unique_user_name.sql'),
      'utf8'
    );
    ...
    console.log('✓ Migration 016 abgeschlossen: Benutzernamen sind jetzt eindeutig.');
  } catch (error: any) {
    ...
    console.error('\nMigration 016 nicht ausgefuehrt:');
